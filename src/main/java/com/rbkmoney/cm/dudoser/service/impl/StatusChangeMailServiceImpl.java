...
public class StatusChangeMailServiceImpl implements MailService<ClaimStatusChanged> {
    ...
    private String convertStatus(ClaimStatus tClaimStatus) {
        com.rbkmoney.cm.dudoser.domain.ClaimStatus claimStatus = convertClaimStatus(tClaimStatus);
        return resolve(claimStatus);
    }

    private com.rbkmoney.cm.dudoser.domain.ClaimStatus convertClaimStatus(ClaimStatus claimStatus) {
        return com.rbkmoney.cm.dudoser.domain.ClaimStatus.valueOf(claimStatus.getSetField().getFieldName());
    }

    private String resolve(com.rbkmoney.cm.dudoser.domain.ClaimStatus claimStatus) {
        switch (claimStatus) {
            case denied:
                return "Отклонена";
            case review:
